...
const Customers = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCustomer) {
        await axios.patch(`http://localhost:3000/api/customers/${editingCustomer.id}`, formData);
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-2xl font-black text-text-main tracking-tighter uppercase italic leading-none">
              Oftsy <span className="text-brand">Clients</span>
            </h1>
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={c.id} className="group hover:bg-surface transition-all">
                  <td className="px-8 py-5 bg-panel/30 first:rounded-l-2xl border-y-2 border-l-2 border-transparent group-hover:border-border-oftsy border-l-transparent group-hover:bg-surface">
                    <div className="flex items-center gap-4">
                  ...
                  <td className="px-8 py-5 bg-panel/30 border-y-2 border-transparent group-hover:border-border-oftsy group-hover:bg-surface">
                  ...
                  <td className="px-8 py-5 bg-panel/30 last:rounded-r-2xl border-y-2 border-r-2 border-transparent group-hover:border-border-oftsy group-hover:bg-surface text-right border-r-transparent">
                    <div className="flex items-center justify-end gap-3">
                      <button onClick={() => openEdit(c)} className="p-3 bg-surface text-text-muted hover:text-brand hover:border-brand border-2 border-border-oftsy rounded-xl transition-all shadow-sm">
                        <Edit3 size={18} />
                      </button>
                      <button onClick={(e) => deleteCustomer(c.id, e)} className="p-3 bg-surface text-text-muted hover:text-red-500 hover:border-red-500 border-2 border-border-oftsy rounded-xl transition-all shadow-sm">
                        <Trash2 size={18} />
                      </button>
                    </div>
                  </td>
             ...
             <form onSubmit={handleSubmit} className="p-10 space-y-8">
                ...
                <div className="space-y-2">
                   ...
                   <input 
                     ...
                     className="w-full bg-panel border-2 border-border-oftsy rounded-2xl py-4 px-6 outline-none focus:border-brand font-black text-xs lowercase"
                     required
                   />
                </div>
                <div className="space-y-2">
