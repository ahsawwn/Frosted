...
// Added 'include' so the frontend can see the unit name/short code
router.get('/', async (req, res) => {
  try {
    const ingredients = await prisma.ingredient.findMany({ 
      include: { unit: true }, // Join with Unit table
      orderBy: { name: 'asc' } 
    });
...
router.post('/recipe', async (req, res) => {
  ...
  try {
    // Transaction ensures we don't delete the old recipe if the new one fails
    await prisma.$transaction([
      // Delete old recipe items for this product
      prisma.recipeItem.deleteMany({
        where: { productId }
      }),
      // Create the new recipe items with unitId
      prisma.recipeItem.createMany({
        data: ingredients.map((item: any) => ({
          productId: productId,
          ingredientId: item.ingredientId,
          quantity: parseFloat(item.quantity),
          unitId: item.unitId, // This must exist in your Unit table
        })),
      })
    ]);
...
// 3. CREATE NEW INGREDIENT (For Warehouse)
  ...
  try {
    const ingredient = await prisma.ingredient.create({
      data: {
        ...
        unitId: unitId // Linking to the Unit model
      }
    });
    res.json(ingredient);
  } catch (error) {
...
router.patch('/:id', async (req, res) => {
  ...
  try {
    const updated = await prisma.ingredient.update({
      ...
      data: { 
        name, 
        unitId, // Update the relationship
