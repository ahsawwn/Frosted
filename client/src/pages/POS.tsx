...
const POS = () => {
  ...
  useEffect(() => {
    const syncCustomerDisplay = async () => {
      try {
        await api.post('/customer-display/sync', {
          items: cart,
          totals: { subtotal, taxAmount, discount: discount + couponDiscount, total }
        });
      } catch (err) {
    ...
    syncCustomerDisplay();
  }, [cart, subtotal, taxAmount, discount, couponDiscount, total]);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Update Customer Display to THANK_YOU
      await api.post('/customer-display/sync', {
        items: cart,
        totals: { subtotal, taxAmount, discount: discount + couponDiscount, total },
        status: 'THANK_YOU'
      ...
      // Reset Customer Display after delay
      setTimeout(async () => {
        await api.post('/customer-display/sync', {
          items: [],
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex flex-col gap-5 mb-6 shrink-0">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-3xl font-black text-text-main tracking-tighter uppercase italic">frosted Frosted Sales</h2>
            ...
            <div className="flex items-center gap-4">
              <div className="bg-panel border-2 border-border-frosted rounded-2xl px-5 py-3 flex items-center gap-4 group">
          ...
          <div className="flex gap-3">
            <div className="relative flex-1 group">
              ...
              <input
                ...
                className="w-full bg-surface border-2 border-border-frosted rounded-2xl py-4 pl-14 pr-6 outline-none focus:border-brand font-black text-xs tracking-wide transition-all placeholder:text-text-muted/40 uppercase"
                value={searchTerm}
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar py-1">
            <button
              ...
              className={`px-6 py-2.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] transition-all border-2 ${activeCategory === 'all' ? 'bg-brand text-white border-brand shadow-glow' : 'bg-surface text-text-muted border-border-frosted hover:border-brand'}`}
            >
              All Categories
            </button>
            {categories.map(cat => (
              <button
                ...
                className={`px-6 py-2.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] transition-all border-2 whitespace-nowrap ${activeCategory === cat.id ? 'bg-brand text-white border-brand shadow-glow' : 'bg-surface text-text-muted border-border-frosted hover:border-brand'}`}
              >
                {cat.name}
              </button>
            ))}
        ...
        <div className="flex-1 overflow-y-auto pr-3 no-scrollbar relative z-10">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full opacity-20 border-2 border-dashed border-border-frosted rounded-[3rem]">
