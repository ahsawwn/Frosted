...
const ReceiptModal = ({ isOpen, onClose, orderData }: any) => {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `frosted_Invoice_${orderData?.id?.slice(-6)}`,
    pageStyle: `
  ...
  const downloadPDF = async () => {
    ...
    pdf.save(`frosted-Invoice-${orderData.id.slice(-4)}.pdf`);
  };
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-[#0a0a0b]/90 backdrop-blur-xl p-4 animate-in fade-in duration-500 font-lexend">
      ...
      <div className="bg-surface w-full max-w-lg rounded-[2.5rem] shadow-glow overflow-hidden flex flex-col border-2 border-brand/20 relative max-h-[95vh]">
        ...
        <div className="flex justify-between items-center p-6 border-b border-white/5 relative z-10 shrink-0">
           ...
           <button onClick={onClose} className="p-3 bg-panel rounded-xl text-text-muted hover:text-brand transition-all border-2 border-border-frosted">
              <X size={20} />
           </button>
        </div>
          ...
          <div className="flex justify-center w-full">
            ...
            >
              <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                 <h1 style={{ margin: '0', fontSize: '24px', fontWeight: '900', letterSpacing: '-1.5px', textTransform: 'uppercase', color: '#000' }}>frosted</h1>
              ...
              <div style={{ marginTop: '30px', textAlign: 'center', borderTop: '1px solid #000', paddingTop: '10px' }}>
                 ...
                 <div style={{ margin: '15px 0', display: 'flex', justifyContent: 'center' }}>
                    <QRCodeSVG 
                      value={`https://frosted.io/track/${orderData.id}`}
                      size={60}
                      level="M"
                      includeMargin={false}
                    />
        ...
        <div className="p-5 bg-surface border-t border-white/5 grid grid-cols-3 gap-4 shrink-0 relative z-10">
          <button onClick={downloadPDF} className="flex flex-col items-center gap-2 p-4 bg-panel rounded-[1.5rem] border-2 border-border-frosted hover:bg-surface hover:border-brand transition-all group">
          ...
          <button className="flex flex-col items-center gap-2 p-4 bg-panel rounded-[1.5rem] border-2 border-border-frosted hover:bg-surface hover:border-brand transition-all group">
